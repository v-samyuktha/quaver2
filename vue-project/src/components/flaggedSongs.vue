<template>
<div v-if="songs" class="container" style="margin-top: 2%; margin-bottom: 0%;">
            <h3><strong>Flagged Songs</strong></h3>
    </div>
        ...
        <div class="d-flex card-container CC-1  col-10" style="height: 38vh; width:100% ;justify-content: left;">
            <div v-for="song in songs" :key="song.song_id" class="card mb-3">
                    ...
                    <div class="card-footer">
                        <button class="btn btn-outline-success" @click="playSong(song.song_id)" disabled>Listen</button>
                        <button class="btn btn-outline-danger" @click="unflagSong(song.song_id)">Un-Flag</button>
                    </div>
            </div>
        </div>
        <div class="arrow-buttons">
...
export default {
    ...
    methods: {
        getSongs() {
            axios.get(`${API_BASE_URL}/songs?flag=true`,{
                headers: {
        ...
        unflagSong(song_id) {
            console.log(`Un-Flagging song ${song_id}`)
